...
function PlatformRow({ platform, onToggle, onDelete }: {
  ...
  const published = platform.status === "PUBLISHED"
    ...
    >
      ...
      <div className="h-9 w-9 shrink-0 overflow-hidden rounded-lg border border-[#E4EAF5] bg-slate-50 flex items-center justify-center">
        {platform.logo ? (
          ...
          <img src={platform.logo} alt={platform.name} className="h-full w-full object-contain p-1" />
        ) : (
          <div
            className="flex h-full w-full items-center justify-center text-xs font-bold text-white"
            style={{ backgroundColor: platform.logoColor || "#3730A3" }}
          >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="truncate font-semibold text-slate-800">{platform.name}</span>
          {platform.featured && <Star size={13} className="shrink-0 fill-amber-400 text-amber-400" />}
        </div>
        <div className="flex items-center gap-2 mt-0.5">
          {platform.category && (
            <span className="text-xs text-slate-400">{platform.category}</span>
          )}
          {platform.pricing && (
            <span className="text-xs text-slate-400">· {platform.pricing}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-1">
        <Link
          href={`/panel/platformlar/${platform.slug}`}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Platformlar</h1>
          <p className="mt-1 text-sm text-slate-500">
            {platforms.length} platform · Sürükleyerek sıralayın
          </p>
        </div>
        <Link
          href="/panel/platformlar/yeni"
      ...
      {platforms.length === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-[#E4EAF5] py-20 text-center">
          <p className="text-sm text-slate-400">Henüz platform eklenmedi.</p>
          <Link
            href="/panel/platformlar/yeni"
        ...
        <Reorder.Group axis="y" values={platforms} onReorder={handleReorder} className="space-y-2">
