...
import { updateSolutionsSectionConfig } from "@/lib/actions/solutions-section"
import type { SolutionsSectionConfig } from "@prisma/client"

interface Props {
  initialData: SolutionsSectionConfig | null
}
...
export default function SolutionsSectionEditor({ initialData }: Props) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await updateSolutionsSectionConfig({ title, subtitle, showCount })
      if (result.success) {
        setSaved(true)
