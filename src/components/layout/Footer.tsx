...
const DEFAULT_SETTINGS: Record<string, string> = {
  ...
  address: "Merkez Mah. Marmara Cad. Bekir Aşçı İş Merkezi No:10, Avcılar / İstanbul",
  linkedin: "https://www.linkedin.com/company/e-partnerim",
  instagram: "https://www.instagram.com/e.partnerim",
  tiktok: "https://www.tiktok.com/@e.partnerim",
  facebook: "https://www.facebook.com/epartnerim",
  footer_description: "İşletmenizin dijital yol arkadaşı. Türkiye'nin önde gelen e-ticaret altyapıları ile iş birliği yapıyoruz.",
};
...
export default function Footer({ settings = DEFAULT_SETTINGS }: Props) {
  ...
  const description = settings.footer_description || DEFAULT_SETTINGS.footer_description;
  const socialLinks = (["linkedin", "instagram", "tiktok", "facebook"] as const)
    .map((key) => ({ key, href: settings[key], label: SOCIAL_LABELS[key], icon: SOCIAL_ICONS[key] }))
    .filter((s) => s.href);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-2 gap-8 md:grid-cols-4 lg:grid-cols-5">
          ...
          <div className="col-span-2 md:col-span-4 lg:col-span-2">
            ...
            <div className="mt-6 space-y-2">
              <a href={`tel:${phone.replace(/\s+/g, "")}`} className="flex items-center gap-2 text-sm text-white/50 hover:text-white/80 transition-colors">
                <span className="text-[#00D084]">T:</span>
                {phone}
              </a>
              <a href={`mailto:${email}`} className="flex items-center gap-2 text-sm text-white/50 hover:text-white/80 transition-colors">
                <span className="text-[#00D084]">E:</span>
                {email}
              </a>
              <p className="text-sm text-white/40 leading-snug max-w-xs">
                {address}
              </p>
            </div>
        ...
        <div className="mt-14 border-t border-white/8 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-white/30">
            © {new Date().getFullYear()} e-partnerim.com — Tüm Hakları Saklıdır.
          </p>
